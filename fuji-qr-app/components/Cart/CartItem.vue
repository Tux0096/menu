...
export default {
  ...
  computed: {
    ...
    totalItemPrice() {
      const modSumPrice = this.item.mods.reduce(
        (acc, el) => acc + Number(el.price ?? 0) * Number(el.amount ?? 1),
        0,
      );
      return ((Number(this.product?.price) || 0) + modSumPrice) * this.item.quantity;
    },
    ...
    displayLinePrice() {
      ...
      if (this.item.displayLinePrice != null) {
        return this.item.displayLinePrice;
      }
...
.cart-item {
  ...
  // .cart-item__desc — компактнее карточки каталога
  &__desc {
    font-size: extClamp(7);
    font-weight: 400;
    line-height: 120%;
    display: -webkit-box;
    overflow: hidden;
