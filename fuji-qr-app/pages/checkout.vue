          ...
          <div class="checkout-page__panel checkout-page__panel--address-footer">
            <CheckoutPanel class="checkout-address-user">
              <AppCheckoutAddress
                v-if="isDelivery"
                :can-edit="true"
                @edit="$store.dispatch('modal/showDeliveryModal')"
              >
                <template #title>
                  Доставка
                </template>
              ...
              <div class="checkout-user">
                ...
                <div class="checkout-user__inner">
                  ...
                  <template v-else>
                    <div class="checkout-user__guest-fields">
                      <BaseInput
                        v-model="user.name"
                        class="checkout-user__input"
                        placeholder="Имя"
                      />
                      <BaseInput
                        v-model="user.phone"
...
import {
  buildRemarkedTransactionMakeOrUpdatePayload,
  postRemarkedTransactionConfirm,
  postRemarkedTransactionMakeOrUpdate,
...
export default {
  ...
  computed: {
    ...
    cartTotalAfterDiscounts() {
      return this.$store.getters['cart/cartTotalAfterDiscountsDisplayed'];
    },
    zoneId() {
  ...
  async created() {
    ...
    const u = this.$store.getters['user/user'] || {};
    this.user = {
      ...u,
      name: u.name || '',
      phone: u.phone || '',
    ...
    async onSending() {
      ...
      try {
        ...
        if (data?.isOrderCreated) {
          const transactionId = Number(data.orderId);
          if (Number.isFinite(transactionId) && transactionId > 0) {
            try {
              const transactionPayload = buildRemarkedTransactionMakeOrUpdatePayload({
                transactionId,
                basket: orderData.order.basket,
                coupon: orderData.coupon || '',
                cartTotal: orderData.order.total,
                withdrawBonuses: orderData.spendBonus || 0,
              });
              await postRemarkedTransactionMakeOrUpdate(transactionPayload);
            } catch (remarkedError) {
              // Remarked-транзакция — надстройка: не блокируем пользовательский флоу заказа.
              // eslint-disable-next-line no-console
              console.warn('[Remarked Loyalty] transaction/makeorupdate error:', remarkedError);
            }
          }
          ...
          const confirmTransaction = async () => {
            ...
            try {
              await postRemarkedTransactionConfirm({ transaction_id: transactionId });
            } catch (remarkedError) {
              // confirm не должен ломать оформление заказа; ошибку оставляем в логах.
              // eslint-disable-next-line no-console
              console.warn('[Remarked Loyalty] transaction/confirm error:', remarkedError);
            }
          };
