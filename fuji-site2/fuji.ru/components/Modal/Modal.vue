...
export default {
  ...
  methods: {
    ...
    hideModal() {
      if (this.currentComponent === 'AppDeliverySelected' && this.$store.getters['cart/deliveryMethod'] === 'self') {
...
.modal {
  ...
  max-height: calc(100vh - var(--mobile-bottom-menu-height) / 2 - var(--mobile-bottom-menu-bottom) - var(--safe-area-inset-bottom, 0) - var(--safe-area-inset-top, 0));
  margin-right: auto;
  margin-left: auto;
...
.modal-checkout-select-address {

  .modal__content-wrapper {
    padding-bottom: extClamp(7);
