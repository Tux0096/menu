<template>
  <div
    :class="{'layout--has-layout-shadow-bottom': $store.state.view.hasLayoutShadowBottom}"
...
import TheMobileBottomMenu from '~/components/TheMobileBottomMenu.vue';
import TheFooter from '~/components/TheFooter.vue';
import StoriesIndex from '~/components/Stories/StoriesIndex.vue';
...
export default {
  ...
  computed: {
    ...
    isMobileBottomMenuHidden() {
      ...
      const EXCLUDED_PAGES = ['cart', 'checkout', 'complete', 'complete-error', 'advent-2025'];
    ...
    isShowModal() {
      return this.$store.getters['modal/isShowModal'];
    },
    isScrollPageHidden() {
      return this.$store.getters['view/isScrollPageHidden'];
    },
  },
  ...
  methods: {
    ...
    updateBodyClasses() {
      if (process.client) {
        ...
        body.classList.remove('scroll-hidden', 'is-modal-show');

        if (this.$store.getters['view/isScrollPageHidden']) {
          body.classList.add('scroll-hidden');
        }
        if (this.$store.getters['modal/isShowModal']) {
          body.classList.add('scroll-hidden', 'is-modal-show');
        }
      }
    },
