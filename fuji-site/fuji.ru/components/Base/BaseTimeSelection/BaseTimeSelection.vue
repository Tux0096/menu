  ...
  methods: {
    ...
    generateTimeRange(currentDate, endDate, timeShift = 120, intervalMinutes = 30, maxEndTime = null) {
      ...
      if (isDecember31 && maxEndTime) {
        // Парсим maxEndTime (формат 'HH:mm')
        const [maxEndHours, maxEndMinutes] = maxEndTime.split(':')
          .map(Number);
        const lastIntervalEnd = new Date(originalDate);
        lastIntervalEnd.setHours(maxEndHours, maxEndMinutes, 0, 0);

        // Время начала последнего интервала (за 2 часа до maxEndTime)
        const lastIntervalStart = new Date(lastIntervalEnd);
        lastIntervalStart.setHours(lastIntervalStart.getHours() - 2);

        // Генерируем интервалы по 2 часа до начала последнего интервала
        while (currentDate < lastIntervalStart) {
          ...
          // Если следующий интервал превышает начало последнего интервала, останавливаемся
        ...
        // Добавляем последний интервал используя maxEndTime
        const lastStartHours = lastIntervalStart.getHours()
          .toString()
          .padStart(2, '0');
        const lastStartMinutes = lastIntervalStart.getMinutes()
          .toString()
          .padStart(2, '0');
        result.push({
          from: `${lastStartHours}:${lastStartMinutes}`,
          to: maxEndTime,
        });
      } else {
    ...
    getNextSevenDays() {
      ...
      if (today.getHours() < 22) {
        ...
        // Проверяем, является ли сегодня 31 декабря
        const isTodayDecember31 = todayCopy.getMonth() === 11 && todayCopy.getDate() === 31;
        ...
        if (isTodayDecember31) {
          todayConfig.intervalMinutes = 120; // 2 часа вместо 30 минут
          todayConfig.maxEndTime = '19:00'; // Ограничение до 17:00-19:00
        }

        result.push(todayConfig);
      }
