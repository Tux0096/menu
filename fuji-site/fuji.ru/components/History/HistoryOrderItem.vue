<template>
  ...
  >
    <div class="history-order-item__title">
      <div class="history-order-item__title-inner">
    ...
    <div class="history-order-item__footer">
      ...
      <div class="history-order-item__footer-actions">
        <BaseButton
          v-if="historyOrderItem.receiptUrl"
          class="history-order-item__receipt-btn"
          @click="onReceiptClick(historyOrderItem)"
        >
          Электронный чек
        </BaseButton>
      </div>
...
export default {
  ...
  components: { AddToCartButton, BaseButton },
  props: {
    historyOrderItem: {
      type: Object,
      required: true,
  ...
  methods: {
    ...
    async onReceiptClick(historyOrderItem) {
      try {
        window.open(historyOrderItem.receiptUrl, '_blank');
      } catch (error) {
        console.error('Ошибка при получении чека:', error);

        // Более детализированное сообщение об ошибке
        let errorMessage = 'Не удалось получить электронный чек';
        if (error.response?.status === 404) {
          errorMessage = 'Чек для данного заказа еще не создан';
        } else if (error.response?.status === 403) {
          errorMessage = 'Нет прав для получения чека';
        }
...
.history-order-item {
  ...
  // .history-order-item__title-inner
  &__title-inner {
    display: flex;
    flex-direction: column;
    gap: extClamp(8);
  ...
  // .history-order-item__footer-actions
  &__footer-actions {
    display: flex;
    align-items: center;
    gap: extClamp(12);
  ...
  // .history-order-item__receipt-btn
  &__receipt-btn {
    font-size: extClamp(10);
    font-weight: 500;
    font-style: normal;
